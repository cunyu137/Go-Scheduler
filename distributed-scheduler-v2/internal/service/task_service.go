...
func (s *TaskService) CreateDelayTask(req CreateDelayTaskRequest) (*model.Task, error) {
	...
	task := &model.Task{Name: req.Name, TaskType: model.TaskTypeDelay, ExecuteAt: &execAt, HandlerName: req.HandlerName, Payload: normalizePayload(req.Payload), RetryLimit: req.RetryLimit, TimeoutSeconds: req.TimeoutSeconds, Status: model.TaskStatusActive, NextRunTime: &execAt}
...
func (s *TaskService) CreateCronTask(req CreateCronTaskRequest) (*model.Task, error) {
	...
	task := &model.Task{Name: req.Name, TaskType: model.TaskTypeCron, CronExpr: &cronExpr, HandlerName: req.HandlerName, Payload: normalizePayload(req.Payload), RetryLimit: req.RetryLimit, TimeoutSeconds: req.TimeoutSeconds, Status: model.TaskStatusActive, NextRunTime: &next}
	if err := s.taskRepo.Create(task); err != nil {
		return nil, err
	}
	return task, nil
